...
export const ItemNameSleepMode = ({ name, index }) => {
  ...
  axios
    .get(
      `http://192.168.1.220:9464/workshop/mainScreen/checkIfPlugRegisteredToSleepMode?i_UiIndex=${index}`
    )
    .then((response) => {
  ...
  const toggleRememberPin = () => {
    ...
    !isEnabled
      ? axios
          .get(
            `http://192.168.1.220:9464/workshop/mainScreen/RegisterPlugToSleepMode?i_UiIndex=${index}`
          )
          .then((response) => {
            console.log("added");
          })
