...
export const ItemNameSleepMode = ({name,index}) => {
  ...
  const [isEnabled, setIsEnabled] = useState(data);
    ...
    axios.get(`http://192.168.1.112:9464/workshop/mainScreen/checkIfPlugRegisteredToSleepMode?i_UiIndex=${index}`)
          .then((response) => {
  ...
  console.log(data)
    ...
    const toggleRememberPin = () => {
      ...
      !isEnabled?
      axios.get(`http://192.168.1.112:9464/workshop/mainScreen/RegisterPlugToSleepMode?i_UiIndex=${index}`)
        .then((response) => {
          console.log("added")
        })
           :
